...
2. **Fetch** -- every text URL goes through `/read` when available. `/read` owns the proxy cascade, paywall detection, and platform routing (WeChat, Feishu, PDF, GitHub). Native fetch tools and raw `curl` silently fail on JS-heavy or paywalled sites and skip all of that. If `/read` is missing (Pre-check warned), fall back to native fetch and accept reduced coverage. For video and X, `/read` usually can't reach the content -- use whatever platform-access method the environment defines for that site (tooling is configured globally, e.g. in CLAUDE.md, and changes over time; do not hardcode a tool here). Capture the transcript or full thread text as a saved file, not just the link. If no access method is available for a platform, say so rather than silently dropping the source.
